...
**Linear probe confirms supervised signal exists.** LOO accuracy 72.3%, permutation p=0.015. Unsupervised configs all failed (best p=0.615) but a thin supervised layer finds the boundary. Signal is real but weak — next: more data (option G) and non-linear probes (option H).
